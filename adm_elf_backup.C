...
adm_elf_read_symbols(const char* filename, uint64_t base)
{
  ...
  if(!object.is_open())
    ...
    for(uint16_t sh=0; sh<ehdr.e_shnum; ++sh) {
      ...
      case SHT_SYMTAB:
      case SHT_DYNSYM:
        if((ptr=malloc_ptr(shdr.sh_size))) {
          object.seekg(shdr.sh_offset, std::ios_base::beg);
    ...
    if(shstr) free_ptr(shstr);
    if(strtab) free_ptr(strtab);
    if(dynstr) free_ptr(dynstr);
    if(symtab) free_ptr(symtab);
    if(dynsym) free_ptr(dynsym);
